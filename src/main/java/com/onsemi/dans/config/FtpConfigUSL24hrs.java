 ...
 */
package com.onsemi.dans.config;

import com.onsemi.dans.dao.EmailConfigDAO;
import com.onsemi.dans.dao.EmailRootcauseTimelapseDAO;
import com.onsemi.dans.dao.EmailTimelapseDAO;
//import com.onsemi.dans.dao.HastMotherboardDAO;
//import com.onsemi.dans.dao.WhRequestDAO;
//import com.onsemi.dans.dao.WhStatusLogDAO;
//import com.onsemi.dans.dao.WhUslReportDAO;
import com.onsemi.dans.model.EmailConfig;
import com.onsemi.dans.model.EmailRootcauseTimelapse;
import com.onsemi.dans.model.EmailTimelapse;
//import com.onsemi.dans.model.HastMbUploadTemp;
//import com.onsemi.dans.model.HastMotherboard;
//import com.onsemi.dans.model.WhStatusLog;
//import com.onsemi.dans.model.WhUslReport;
...
//    @Scheduled(cron = "0 0 */6 * * *") //every 6 hour !! inactive !!
//    @Scheduled(cron = "0 16 17 * * ?") //every 8:00 AM - cron (sec min hr daysOfMth month daysOfWeek year(optional)) !! inactive !!
...
//        String reportName = "C:\\Users\\" + username + "\\Documents\\CDARS\\HIMS USL for Sending to SBN Factory Report (" + todayDate + ").xls";
        String reportName = "\\\\mysed-rel-app03\\d$\\HIMS\\RL\\HIMS USL for Sending to SBN Factory Report (" + todayDate + ").xls";
...
//            List<WhUSL> whUslList = whUslDAO.getWhUSLLog();
        ...
        /* COMMENT BECAUSE WE WANT TO CHANGE THIS THING
...
//            String[] to = {"fg79cj@onsemi.com"};
            emailSender.htmlEmailWithAttachment(
                    ...
                    new File("\\\\mysed-rel-app03\\d$\\HIMS\\RL\\HIMS USL for Sending to SBN Factory Report (" + todayDate + ").xls"),
...
//     @Scheduled(cron = "0 0 */6 * * *") //every 6 hour inactive
//    @Scheduled(cron = "0 14 17 * * ?") //every 8:00 AM - cron (sec min hr daysOfMth month daysOfWeek year(optional)) inactive
...
//        String reportName = "C:\\Users\\" + username + "\\Documents\\CDARS\\HIMS USL for Retrieving from SBN Factory Report (" + todayDate + ").xls";
        String reportName = "\\\\mysed-rel-app03\\d$\\HIMS\\RL\\HIMS USL for Retrieving from SBN Factory Report (" + todayDate + ").xls";
...
//            String[] to = {"fg79cj@onsemi.com"};
            emailSender.htmlEmailWithAttachment(
                    ...
                    new File("\\\\mysed-rel-app03\\d$\\HIMS\\RL\\HIMS USL for Retrieving from SBN Factory Report (" + todayDate + ").xls"),
...
//    @Scheduled(cron = "0 0 09 01 * ?") //every 1st day of month at 9:00 AM - cron (sec min hr daysOfMth month daysOfWeek year(optional)) active!
...
//        String reportName = "C:\\Users\\" + username + "\\Documents\\CDARS\\SF and SBN Rel - Time Lapse Report Monthly Performance (" + todayDate + ").xls";
        String reportName = "\\\\mysed-rel-app03\\d$\\HIMS\\RL\\Monthly Report\\SF and SBN Rel - Time Lapse Report Monthly Performance (" + todayDate + ").xls";
...
//        }
        ...
        if (countCc > 0) { //use htmlEmailWithAttachmentWithCc
            ...
            emailSender.htmlEmailWithAttachmentWithCc(
                    ...
                    new File("\\\\mysed-rel-app03\\d$\\HIMS\\RL\\Monthly Report\\SF and SBN Rel - Time Lapse Report Monthly Performance (" + todayDate + ").xls"),
...
//    @Scheduled(cron = "0 15 08 * * *") //every 8.15am disable for now. active
...
//        LOGGER.info("cron detected -[Approval Reminder]!!!");
        try {
            ...
            if (countShip > 0 || countRetrieve > 0) {
                ...
                /* CHANGE BECAUSE WE WANT TO CHANGE THIS THING
...
//    @Scheduled(cron = "0 06 11 * * *") //every 8.15am
    public void UploadHastMb() {
...
//            LOGGER.info("username!!!" + username);
            File file = new File("C:\\Users\\fg79cj\\Documents\\HAST Motherboard Data\\upload July 31.csv");

            //            File file = new File("D:\\DOTS\\RMS_FTP\\dots_rms_ftp.csv");
            if (file.exists()) {
//                LOGGER.info("File dots_rms_ftp found!!!");
                csvReader = new CSVReader(new FileReader("C:\\Users\\fg79cj\\Documents\\HAST Motherboard Data\\upload July 31.csv"), ',', '"', 1);
                ...
                List<HastMbUploadTemp> empList = new ArrayList<HastMbUploadTemp>();

                while ((inventory = csvReader.readNext()) != null) {
                    //Save the employee details in Employee object
                    HastMbUploadTemp emp = new HastMbUploadTemp(
                            inventory[0], //rms
                            inventory[1], //interval
                            inventory[2], //prefix
                            inventory[3], //1st board
                            inventory[4],
                            inventory[5],
                            inventory[6],
                            inventory[7],
                            ...
                            inventory[12],
                            inventory[13],
                            inventory[14]); //12th board
                    empList.add(emp);
                }

                int count = 0;
                //Lets print the Inventory List
                for (HastMbUploadTemp e : empList) {

                    //1st mb
                    //check mbID & RMS & intervals already exist or not
                    HastMotherboardDAO countD = new HastMotherboardDAO();
                    HastMotherboard hB1 = new HastMotherboard();
                    int countMb = countD.getCountMbwithSameRmsAndIntervals(e.getFirstBoard(), e.getRms(), e.getInterval());
                    if (countMb == 0) {
                        hB1.setRms(e.getRms());
                        hB1.setPrefix(e.getPrefix());
                        hB1.setInterval(e.getInterval());
                        hB1.setBoardName(e.getFirstBoard());
                        HastMotherboardDAO insertD = new HastMotherboardDAO();
                        QueryResult query1st = insertD.insertHastMotherboard(hB1);
                        if (query1st.getGeneratedKey().equals("0")) {
                            LOGGER.info("Fail to insert 1st mb = " + e.getFirstBoard());
                        } else {
                            count++;
                        }
                    }
                    //2nd mb
                    if (e.getSecondBoard().length() > 1) {
                        hB1 = new HastMotherboard();
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(e.getSecondBoard(), e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            hB1.setRms(e.getRms());
                            hB1.setPrefix(e.getPrefix());
                            hB1.setInterval(e.getInterval());
                            hB1.setBoardName(e.getSecondBoard());
                            HastMotherboardDAO insert2 = new HastMotherboardDAO();
                            QueryResult query2 = insert2.insertHastMotherboard(hB1);
                            if (query2.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 2nd mb = " + e.getSecondBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //3rd mb
                    if (e.getThirdBoard().length() > 1) {
                        String mbName = "";
                        if (e.getThirdBoard().contains("AV") || e.getThirdBoard().contains("TT")
                                || e.getThirdBoard().contains("HA") || e.getThirdBoard().contains("ha")
                                || e.getThirdBoard().contains("HW") || e.getThirdBoard().contains("nt")
                                || e.getThirdBoard().contains("ppd")) {
                            mbName = e.getThirdBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getThirdBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            hB1 = new HastMotherboard();
                            hB1.setRms(e.getRms());
                            hB1.setPrefix(e.getPrefix());
                            hB1.setInterval(e.getInterval());
                            hB1.setBoardName(mbName);
                            HastMotherboardDAO insert3 = new HastMotherboardDAO();
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 3rd mb = " + e.getThirdBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //4th
                    if (e.getFourthBoard().length() > 1) {
                        String mbName = "";
                        if (e.getFourthBoard().contains("AV") || e.getFourthBoard().contains("TT")
                                || e.getFourthBoard().contains("HA") || e.getFourthBoard().contains("ha")
                                || e.getFourthBoard().contains("HW") || e.getFourthBoard().contains("nt")
                                || e.getFourthBoard().contains("ppd")) {
                            mbName = e.getFourthBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getFourthBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            ...
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 4th mb = " + e.getFourthBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //5th
                    if (e.getFifthBoard().length() > 1) {
                        String mbName = "";
                        if (e.getFifthBoard().contains("AV") || e.getFifthBoard().contains("TT")
                                || e.getFifthBoard().contains("HA") || e.getFifthBoard().contains("ha")
                                || e.getFifthBoard().contains("HW") || e.getFifthBoard().contains("nt")
                                || e.getFifthBoard().contains("ppd")) {
                            mbName = e.getFifthBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getFifthBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            ...
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 5th mb = " + e.getFifthBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //6th
                    if (e.getSixthBoard().length() > 1) {
                        String mbName = "";
                        if (e.getSixthBoard().contains("AV") || e.getSixthBoard().contains("TT")
                                || e.getSixthBoard().contains("HA") || e.getSixthBoard().contains("ha")
                                || e.getSixthBoard().contains("HW") || e.getSixthBoard().contains("nt")
                                || e.getSixthBoard().contains("ppd")) {
                            mbName = e.getSixthBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getSixthBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            ...
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 6th mb = " + e.getSixthBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //7th
                    if (e.getSeventhBoard().length() > 1) {
                        String mbName = "";
                        if (e.getSeventhBoard().contains("AV") || e.getSeventhBoard().contains("TT")
                                || e.getSeventhBoard().contains("HA") || e.getSeventhBoard().contains("ha")
                                || e.getSeventhBoard().contains("HW") || e.getSeventhBoard().contains("nt")
                                || e.getSeventhBoard().contains("ppd")) {
                            mbName = e.getSeventhBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getSeventhBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            ...
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 7th mb = " + e.getSeventhBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //8th
                    if (e.getEightBoard().length() > 1) {
                        String mbName = "";
                        if (e.getEightBoard().contains("AV") || e.getEightBoard().contains("TT")
                                || e.getEightBoard().contains("HA") || e.getEightBoard().contains("ha")
                                || e.getEightBoard().contains("HW") || e.getEightBoard().contains("nt")
                                || e.getEightBoard().contains("ppd")) {
                            mbName = e.getEightBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getEightBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            ...
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 8th mb = " + e.getEightBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //9th
                    if (e.getNinthBoard().length() > 1) {
                        String mbName = "";
                        if (e.getNinthBoard().contains("AV") || e.getNinthBoard().contains("TT")
                                || e.getNinthBoard().contains("HA") || e.getNinthBoard().contains("ha")
                                || e.getNinthBoard().contains("HW") || e.getNinthBoard().contains("nt")
                                || e.getNinthBoard().contains("ppd")) {
                            mbName = e.getNinthBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getNinthBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            ...
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 9th mb = " + e.getNinthBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //10th
                    if (e.getTenthBoard().length() > 1) {
                        String mbName = "";
                        if (e.getTenthBoard().contains("AV") || e.getTenthBoard().contains("TT")
                                || e.getTenthBoard().contains("HA") || e.getTenthBoard().contains("ha")
                                || e.getTenthBoard().contains("HW") || e.getTenthBoard().contains("nt")
                                || e.getTenthBoard().contains("ppd")) {
                            mbName = e.getTenthBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getTenthBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            ...
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 10th mb = " + e.getTenthBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //11th
                    if (e.getEleventhBoard().length() > 1) {
                        String mbName = "";
                        if (e.getEleventhBoard().contains("AV") || e.getEleventhBoard().contains("TT")
                                || e.getEleventhBoard().contains("HA") || e.getEleventhBoard().contains("ha")
                                || e.getEleventhBoard().contains("HW") || e.getEleventhBoard().contains("nt")
                                || e.getEleventhBoard().contains("ppd")) {
                            mbName = e.getEleventhBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getEleventhBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            ...
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 11th mb = " + e.getEleventhBoard());
                            } else {
                                count++;
                            }
                        }
                    ...
                    //12th
                    if (e.getTwelvethBoard().length() > 1) {
                        String mbName = "";
                        if (e.getTwelvethBoard().contains("AV") || e.getTwelvethBoard().contains("TT")
                                || e.getTwelvethBoard().contains("HA") || e.getTwelvethBoard().contains("ha")
                                || e.getTwelvethBoard().contains("HW") || e.getTwelvethBoard().contains("nt")
                                || e.getTwelvethBoard().contains("ppd")) {
                            mbName = e.getTwelvethBoard();
                        } else {
                            mbName = e.getPrefix() + "-" + e.getTwelvethBoard();
                        }
                        countD = new HastMotherboardDAO();
                        countMb = countD.getCountMbwithSameRmsAndIntervals(mbName, e.getRms(), e.getInterval());
                        if (countMb == 0) {
                            ...
                            QueryResult query3 = insert3.insertHastMotherboard(hB1);
                            if (query3.getGeneratedKey().equals("0")) {
                                LOGGER.info("Fail to insert 12th mb = " + e.getTwelvethBoard());
                            } else {
                                count++;
                            }
                        }
